...
import { AiOutlineClose, AiOutlineLoading3Quarters, AiOutlineMinusSquare, AiOutlinePlusSquare } from "react-icons/ai";
...
const DomainBar: React.FC<DomainBarProps> = ({ name, type, content }) => {
  ...
  async function Ping(domain: string, type: string): Promise<ResultProps> {
    try {
      const result = await fetch(`/api/nslookup/${type}/${domain}`);
      return result.json();
    } catch (error) {
